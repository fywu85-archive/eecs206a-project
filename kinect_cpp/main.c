...
void print_body_information(k4abt_body_t body, FILE * pFile)
{
    SYSTEMTIME time;
    GetSystemTime(&time);
    LONG ms = (time.wMinute * 60 * 1000) + (time.wSecond * 1000) + time.wMilliseconds;
    ...
    for (int i = 0; i < (int)K4ABT_JOINT_COUNT; i++)
    {
        ...
        fprintf(pFile, "Time[ms] %d; Joint[%d]: Position[mm] ( %f, %f, %f ); Orientation ( %f, %f, %f, %f); Confidence Level (%d) \n",
            ms, i, position.v[0], position.v[1], position.v[2], orientation.v[0], orientation.v[1], orientation.v[2], orientation.v[3], confidence_level);
    }
}
...
{
    FILE* pFile;
    errno_t error_code = fopen_s(&pFile, "data.txt", "w");
        ...
        if (get_capture_result == K4A_WAIT_RESULT_SUCCEEDED)
        {
            ...
            if (pop_frame_result == K4A_WAIT_RESULT_SUCCEEDED)
            {
                ...
                for (uint32_t i = 0; i < num_bodies; i++)
                {
                    ...
                    print_body_information(body, pFile);
                }
